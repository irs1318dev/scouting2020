...
import { MatchScoreCard, Measure } from "./match";
...
export class HeroService {
  ...
  getRedVBlue(): Observable<RedVBlue> {
    return this.http.get<RedVBlue>(this.matchTeamsUrl).pipe(
      tap((_) => this.log("fetched heroes")),
      catchError(this.handleError<RedVBlue>("getHeroes"))
    );
  }

  clearMeasures(matchScoreCard: MatchScoreCard): void {
    if (this.measures == null || this.measures == null) {
      localStorage.removeItem(this.getMatchKey(matchScoreCard));
      return;
    }
    localStorage.setItem(
      this.getMatchKey(matchScoreCard),
  ...
  saveMeasures(matchScoreCard: MatchScoreCard): void {
    if (this.measures == null || this.measures == null) {
      localStorage.removeItem(this.getMatchKey(matchScoreCard));
      return;
    }
  ...
  private getMatchKey(matchScoreCard: MatchScoreCard) {
    return `match-${matchScoreCard.match.name}-${matchScoreCard.selectedTeam.name}`;
  }
    ...
    if (this.measures == null || this.measures == undefined) {
      if (localStorage.getItem(this.getMatchKey(matchScoreCard))) {
        this.measures = JSON.parse(
          localStorage.getItem(this.getMatchKey(matchScoreCard))
        ) as Measure[];
      } else {
        this.measures = MEASURES;
      }
  ...
  public startMatch(team: string, match: string): Observable<Measure[]> {
    this.measures = MEASURES;
    this.getScore(team, match).subscribe((c) =>
      c.forEach((rec) => this.assignMeasure(rec))
    );
    return of(this.measures);
  }

  assignMeasure(record: ScoreRecord) {
    let matchedMeasure = this.measures.find(
      (m) =>
        m.task_name === record.task && PhasesLabel.get(m.phase) === record.phase
    );
    if (matchedMeasure) {
  ...
  public postScore(team: string, match: string): void {
    for (let meas of this.measures) {
      var urlString = `${this.updateScoreUrl}/${match}/${team}/${
        meas.task_name
      }/${phases[meas.phase]}/${meas.capability}/${this.getAttempts(
        meas.attempts
      )}/${meas.successes}`;
  ...
  getScore(team: string, match: string): Observable<ScoreRecord[]> {
    let results = this.http
      .get<ScoreRecord[]>(`${this.scoreUrl}/${team}/${match}`)
      .pipe(
  ...
  getBlankMeasures(matchScoreCard: MatchScoreCard): Observable<Measure[]> {
    this.messageService.add("HeroService: fetched Measures");
    this.populateMeasures(matchScoreCard);
